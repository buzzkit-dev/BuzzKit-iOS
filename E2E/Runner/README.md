...
| `local-fallback.test.ts` | A `deliver: local` send behind a one-minute `waitUntil` with real production durations: the device never receives the plan, the run wakes at the window, records the fallback and the reminder arrives as an ordinary APNs push (`willPresent` with the fallback message id, delivery settled). When the simulator does receive the silent plan and acknowledges it, the scenario asserts the local path instead and says so |
| `canary.test.ts` | No simulator. Keeps one permanent `e2e-canary` subscriber, segment and daily-scheduled workflow (04:20 UTC, a three-hour `wait`, then a `set`) in the tenant, and asserts on every run that the schedule fired inside the last 26 hours and that every run older than four hours completed on its last step, so a regression in hibernation, Durable Object eviction or the scheduler shows up within a day |
